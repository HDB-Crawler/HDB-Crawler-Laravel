...
class ActivityController extends Controller
{
	public function getDashboard()
	{
		...
		return view('dashboard', ['activities' => $activity]);
	}
	public function postCreateActivity(Request $request)
	{
		...
		$activity->start_time = $request['start-time'];
		$activity->end_time = $request['end-time'];
		$activity->description = $request['description'];
		$activity->members_attending = 0;
		$activity->sentiment = 0;
	...
	public function apiGetActivities()
	{
		$activity = Activity::all();
		return response()->json($activity);
	}

	public function postApiSetFeedback()
	{
		$feedback = new Feedback();
		$feedback['activity_id'] = 0;
		$feedback['user_id'] = 0;
		...
		$data = array(
		  "note" => "I hate this.",
		  "apikey" => "bcd0f48ef2ed9be46f9fa2304691e80ed678d6c1"
		);
		$url_send = "https://gateway-a.watsonplatform.net/calls";
		$str_data = json_encode($data);
		echo " " . ActivityController::sendPostData($url_send, $str_data);
		*/
		// require_once 'alchemyapi.php';
		$alchemyapi = new AlchemyAPI("bcd0f48ef2ed9be46f9fa2304691e80ed678d6c1");
		//$myText = $data['message'];
		$myText = $data['message'];
		...
		$result = $response["docSentiment"]["type"];
		if ($result == 'negative')
		{
			$activity = Activity::where('id', '1')->first();
        	$activity->sentiment -= 5;
        	$activity->save();
		} else if ($result == 'positive')
		{
			$activity = Activity::where('id', '1')->first();
        	$activity->sentiment += 1;
        	$activity->save();
		} else {
